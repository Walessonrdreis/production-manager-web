...
**Versão:** v4.6.3 (Atualizado em 08/05/2026 - Padronização da Sincronização via Cron/Background Jobs no Backend)
...
### 0.5. Atualização de Sincronização e Refresh via Cron Job (v4.6.3 - Atual)
...
- **Correção Endpoint Admin Orders:** Foi ajustado o consumo da rota remota para utilizar `/v1/admin/orders` no `OrdersAdapter` garantindo o sync de Ordens de Venda de maneira consistente e limitando a requisição para batches de `pageSize=200`.
