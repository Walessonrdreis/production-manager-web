...
**Versão:** v4.6.4 (Atualizado em 09/05/2026 - Correção Bug Firebase de Retenção de Dados Obsoletos / Delete)
...
### 0.5. Atualização de Sincronização e Refresh via Cron Job (v4.6.4 - Atual)
- **Background Sync a cada 10 min:** Foi implementado no bootstrap da API (node-cron) a execução contínua `startBackgroundJobs()` que atualiza os dados do Render e sincroniza nativamente com o Firebase (para Pedidos, Produtos e Clientes).
...
- **Correção Endpoint Admin Orders e Memory Leak:** Ajuste na URL de consumo do `/v1/admin/orders` limitando `pageSize=200`, além da correção do limit de EventEmitter resolvendo o bug do `MaxListenersExceededWarning` injetando `setMaxListeners` apropriados pelo `externalClient`.
