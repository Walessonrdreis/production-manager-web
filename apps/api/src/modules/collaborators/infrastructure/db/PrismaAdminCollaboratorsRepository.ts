...
export class PrismaAdminCollaboratorsRepository {
  ...
  static async save(collaborator: any) {
    try {
      ...
      const saved = await prisma.collaborator.create({
        data: {
          id,
  ...
  static async update(id: string, collaborator: any) {
    try {
      const dataToUpdate: any = { ...collaborator };
      if (collaborator.metrics) dataToUpdate.metrics = JSON.stringify(collaborator.metrics);
