...
export function CatalogTable({
  ...
  return (
    <div className="flex flex-col gap-2 pb-4">
      <div className="border border-slate-200 bg-slate-50 px-3 py-2 flex items-center justify-between rounded-xl mb-2 shadow-sm">
        <button
          onClick={onToggleSelectAll}
          className="flex items-center text-xs font-semibold text-slate-600 hover:text-blue-700 transition-colors bg-white hover:bg-blue-50 px-3 py-1.5 rounded-lg border border-slate-200 shadow-sm"
        >
          {allPageSelected ? (
            <>
        ...
        {products.map((p) => {
          ...
          return (
            <div
              key={p.id}
              className={cn(
                "group flex flex-col md:flex-row md:items-center justify-between p-3 md:px-4 gap-3 md:gap-4 bg-white border border-slate-200 rounded-xl shadow-sm hover:shadow-md transition-all cursor-pointer relative",
                isSelected
            ...
            >
              ...
              <div className="flex items-center gap-3 w-full md:w-[45%] min-w-0 shrink">
                ...
                <div className="flex flex-col gap-1.5 min-w-0 flex-1">
                  ...
                  <h3
                    className="font-bold text-slate-900 group-hover:text-blue-700 transition-colors uppercase leading-tight text-sm truncate"
                    title={p.description}
                  >
                    {p.description}
                  </h3>
              ...
              <div className="flex flex-col items-start w-full md:w-[20%] shrink-0 min-w-0">
                {p.family && (
                  <span className="text-[10px] font-bold text-slate-500 uppercase leading-none px-2 py-1 bg-slate-100 rounded-md truncate max-w-full">
                    Família: {p.family}
                  </span>
                )}
              </div>
              ...
              <div className="flex flex-wrap sm:flex-nowrap items-center justify-between w-full md:w-[35%] pt-3 border-t border-slate-100 md:border-t-0 md:pt-0 gap-3 shrink-0">
                ...
                <div className="flex items-center justify-end shrink-0 ml-auto bg-slate-50 md:bg-transparent rounded-lg p-1 md:p-0">
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                    ...
                    className={cn(
                      ...
                      saved
                        ...
                        : "text-slate-500 md:text-slate-400 bg-slate-100 md:bg-transparent border border-transparent md:border-slate-200 hover:text-blue-600 hover:bg-blue-50 hover:border-blue-200",
                    )}
                    title={
