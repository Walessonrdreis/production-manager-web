...
export function OrderStatusBadge({ status }: OrderStatusBadgeProps) {
  const map: Record<ProductionOrderV2['status'], { label: string; color: string }> = {
    OPENED: { label: 'Aberto', color: 'bg-blue-100 text-blue-800 border-blue-200' },
    LIVE: { label: 'Em Execução', color: 'bg-emerald-100 text-emerald-800 border-emerald-200' },
    REVIEW: { label: 'Revisão', color: 'bg-amber-100 text-amber-800 border-amber-200' },
    HISTORY: { label: 'Concluído', color: 'bg-gray-100 text-gray-800 border-gray-200' },
