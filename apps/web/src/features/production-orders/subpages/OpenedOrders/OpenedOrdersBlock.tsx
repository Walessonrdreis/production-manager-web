...
export function OpenedOrdersBlock() {
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-200 dark:border-slate-800 overflow-hidden hover:border-blue-500 dark:hover:border-blue-600 transition-colors">
      ...
      <div className="p-5 bg-white dark:bg-slate-900 flex-1 flex flex-col items-center justify-center">
        {isLoading ? (
          ...
          <div className="text-center">
            <span className="text-4xl font-bold text-gray-800 dark:text-slate-100">{orders?.length || 0}</span>
            <span className="block text-sm font-medium text-gray-500 dark:text-gray-400 mt-1 uppercase tracking-wide">em aberto</span>
          </div>
        )}
      </div>
    </div>
