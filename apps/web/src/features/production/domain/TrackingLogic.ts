...
export const TrackingLogic = {
  ...
  aggregateStage20Totals(rawData: any): DashboardTotalsResponse {
    console.log('[TrackingLogic] Início da agregação. RawData:', rawData);
    if (!rawData) {
      console.warn('[TrackingLogic] Nenhum dado recebido');
      return { data: [], totalItems: 0 };
    }

    // Adaptar para a nova estrutura { sucess: true, data: [...] } ou array direto
    let rawProducts = [];
    ...
    const products = rawProducts.map((p: any) => ({
      description: p.description || p.descricao || p.descr || 'Sem descrição',
      totalQuantity: Number(p.totalQuantity || p.total_quantidade || p.quantidade || p.nQtde || p.nQtdeTotal || 0)
    }));

    const totalUnits = products.reduce((acc: number, curr: any) => acc + curr.totalQuantity, 0);

    console.log(`[TrackingLogic] Agregação concluída. Produtos: ${products.length}, Unidades: ${totalUnits}`);
