...
export function ProductDetailsModal({ product, demandMap = {}, isOpen, onClose, onPlanProduct }: ProductDetailsModalProps) {
  ...
  const [inlineEditingPeriod, setInlineEditingPeriod] = useState<GoalPeriod | null>(null);
  const [inlineTargetQty, setInlineTargetQty] = useState<number>(0);
  ...
  const productGoals = useMemo(() => {
    ...
    return goals.filter(g => 
      (g.productCode && product.code && g.productCode === product.code) || 
      (g.productDescription && product.description && g.productDescription === product.description) ||
      (g.productCode === product.id)
    );
  }, [product, goals]);
  ...
  const handleSaveInlineGoal = async (p: GoalPeriod) => {
    try {
      const existing = productGoals.find(g => g.period === p);
      if (existing) {
        if (inlineTargetQty <= 0) {
          await deleteGoal(existing.id);
        } else {
          await updateGoal(existing.id, { targetQuantity: inlineTargetQty });
        }
      } else {
        if (inlineTargetQty > 0) {
          await saveGoal({
            productCode: String(product.code || product.id),
            productDescription: product.description,
            targetQuantity: inlineTargetQty,
            period: p,
            isActive: true
          });
        }
                      ...
                      <div>
                        ...
                        {isEditing ? (
                          <div className="flex items-center space-x-1">
                            <input
                              type="number"
                              className="w-20 px-1.5 py-0.5 text-sm font-bold border border-indigo-300 rounded text-indigo-700 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                              value={inlineTargetQty === 0 ? '' : inlineTargetQty}
                              onChange={(e) => setInlineTargetQty(Number(e.target.value))}
                              autoFocus
                              onKeyDown={(e) => {
                                if (e.key === 'Enter') handleSaveInlineGoal(p);
                                if (e.key === 'Escape') setInlineEditingPeriod(null);
                              }}
                              onBlur={() => handleSaveInlineGoal(p)}
                            />
                          ...
                          <div 
                            ...
                            onClick={() => {
                              setInlineEditingPeriod(p);
                              setInlineTargetQty(goal ? goal.targetQuantity : 0);
                            }}
                          >
                            <span className={`text-xl leading-none font-bold ${goal ? 'text-indigo-600' : 'text-slate-400'}`}>
                              {goal ? goal.targetQuantity : 0}
                            </span>
                            <span className="text-[10px] text-slate-400">{product.unit}</span>
                            <Edit2 size={12} className="text-indigo-400 opacity-0 group-hover:opacity-100 transition-opacity ml-1" />
                          </div>
                        )}
                      </div>
                      {!isEditing && goal && (
                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => setGoalToDelete(goal.id)} className="p-1.5 text-slate-400 hover:text-red-600 rounded hover:bg-red-50" title="Remover Meta">
                            <Trash2 size={14} />
                          </button>
                        </div>
                      )}
